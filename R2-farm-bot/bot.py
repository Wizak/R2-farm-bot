...
def main():
    ...
    main_process = False
    need_title = '??'
    mark_hp = resource_path(os.path.join('Mark', 'Hp.png'))
    mark_screen = resource_path(os.path.join('Mark', 'Screenshot.png'))
    eps = [50, -24]
    dtime = 0.15
    death = (10, 10, 10)
    action = []
    window = tk.Tk()
    window.iconbitmap(resource_path(os.path.join('Interface', 'R2-Logo.ico')))
